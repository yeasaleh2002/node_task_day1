...
$(document).on("click", ".beginButton", async function () {
  // New functionality to ask user if they want to resume
  const resume = confirm("Do you want to resume from where you left off?");
  if (resume) {
    // Load the last saved state from localStorage
    const savedState = JSON.parse(localStorage.getItem("quizState"));
    if (savedState) {
      currentQuizIndex = savedState.currentQuizIndex;
      currentQuestionCounter = savedState.currentQuestionCounter;
    }
  } else {
      ...
      if (val.answer) {
        $("#typeSelection .answerInner").append(`
          <div class="selectionOptions">
            <button data-val="${val.answer}" data-id="${val.id}" class="selectionBtns selectionBtn"
            onclick="checkAllergie('${val.answer}')"
            >${val.answer}</button>
          </div>
        `);
      }
