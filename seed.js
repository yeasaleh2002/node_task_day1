...
async function executeSeeds() {
  ...
  await db.sequelize.query('DROP TABLE IF EXISTS `question`');
  
  // Then drop the referenced tables
  await db.sequelize.query('DROP TABLE IF EXISTS `quiz`');
  await db.sequelize.query('DROP TABLE IF EXISTS `user`');
  await db.sequelize.query('DROP TABLE IF EXISTS `token`');
  await db.sequelize.query('DROP TABLE IF EXISTS `sms`');
  await db.sequelize.query('DROP TABLE IF EXISTS `setting`');
  await db.sequelize.query('DROP TABLE IF EXISTS `rule`');
  await db.sequelize.query('DROP TABLE IF EXISTS `role`');
  await db.sequelize.query('DROP TABLE IF EXISTS `result_profile`');
  await db.sequelize.query('DROP TABLE IF EXISTS `refer_log`');
